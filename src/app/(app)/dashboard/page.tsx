...
import { getSettlementStats, type SettlementStats } from "@/services/settlementService";
import { getSubscription, type VendorSubscription } from "@/services/subscriptionService";
...
export default function DashboardPage() {
  ...
  const [settlementStats, setSettlementStats] = React.useState<SettlementStats | null>(null);
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20 overflow-x-hidden w-full max-w-[100vw]">
      ...
      <div className="grid gap-3 md:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <MetricCard
          ...
          value={settlementStats?.cleared ? `ZMW ${settlementStats.cleared.toLocaleString(undefined, { minimumFractionDigits: 2 })}` : "ZMW 0.00"}
          secondaryValue={settlementStats?.pending ? `ZMW ${settlementStats.pending.toLocaleString()} pending` : undefined}
          secondaryLabel=""
          icon={DollarSign}
        ...
        <MetricCard
          ...
          ctaLink={subscription?.status === 'active' || (subscription?.status === 'trial' && new Date(subscription.trial_ends_at!) > new Date()) ? `/reports/profit${queryParams}` : `/subscription`}
          ctaText={subscription?.status === 'active' || (subscription?.status === 'trial' && new Date(subscription.trial_ends_at!) > new Date()) ? "View Profit Report" : "Upgrade to Unlock"}
          className={!(subscription?.status === 'active' || (subscription?.status === 'trial' && new Date(subscription.trial_ends_at!) > new Date())) ? "opacity-75 bg-muted/50 grayscale" : ""}
        />
        <MetricCard
