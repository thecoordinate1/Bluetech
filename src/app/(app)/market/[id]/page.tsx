...
import { Loader2, ArrowLeft, Import, ShoppingBag, Star, Truck, ShieldCheck, Box, Layers, AlertCircle, Info, Tag } from "lucide-react";
...
export default function MarketProductDetailsPage() {
    ...
    const handlePaymentConfirm = async (phone: string, provider: 'airtel' | 'mtn' | 'zamtel') => {
        ...
        try {
            // 1. Initiate Payment
            const payResult = await initiateMarketImportPayment(selectedStoreId, product.id, provider, phone, importFee);
            ...
            // 2. Proceed to Import (Optimistic/Simulated Flow)
            ...
            toast({ title: "Error", description: "Payment processing error.", variant: "destructive" });
        } finally {
            setIsProcessingPayment(false);
            setImporting(false);
        }
    ...
    const handleUseCredit = async () => {
        ...
        try {
            const payResult = await initiateMarketImportPayment(selectedStoreId, product.id, 'free_credit', '0000000000', 0);
            ...
            if (user) {
                ...
                if (error) {
                    ...
                    toast({ title: "Product Imported", description: `${product.name} added to your drafts.` });
                    setImportCredits(prev => Math.max(0, prev - 1));
                    setImporting(true); // Disable import button
                }
            }
        } catch (e: any) {
                    ...
                    <div className="flex flex-col gap-3">
                        {stores.length > 0 ? (
                            <div className="flex gap-3">
                                ...
                                <Button size="lg" className="flex-1 h-12 text-base font-semibold shadow-lg shadow-primary/20" onClick={handleImportClick} disabled={importing}>
                            ...
                            <div className="flex flex-col gap-4">
                                <div className="flex items-start gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm text-muted-foreground">Delivered within 1-7 business days via Entemba Logistics.</p>
                                    </div>
                                </div>
                                <div className="flex items-start gap-3">
