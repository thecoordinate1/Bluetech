...
import { initiateMarketImportPayment } from "@/app/actions/lenco";
import { getImportCreditStats } from "@/services/creditService";
...
export default function MarketPage() {
    ...
    const [productToPay, setProductToPay] = React.useState<any | null>(null);
    const [isProcessingPayment, setIsProcessingPayment] = React.useState(false);
    const [importCredits, setImportCredits] = React.useState<number>(0);
    ...
    React.useEffect(() => {
        const fetchMarketData = async () => {
            ...
            // 3. Fetch Import Credits
            try {
                const stats = await getImportCreditStats(user.id);
                setImportCredits(stats.remaining);
            } catch (e) {
    ...
    const handlePaymentConfirm = async (phone: string, provider: 'airtel' | 'mtn' | 'zamtel') => {
        if (!selectedStoreId || !productToPay) return;

        setIsProcessingPayment(true);
        // User Logic: import cost is 12.5% of the price
        const basePrice = productToPay.supplier_price || productToPay.price;
        const importFee = basePrice * 0.125;

        try {
            // 1. Initiate Payment
            const payResult = await initiateMarketImportPayment(selectedStoreId, productToPay.id, provider, phone, importFee);

            if (!payResult.success) {
                toast({ title: "Payment Failed", description: payResult.message, variant: "destructive" });
                setIsProcessingPayment(false);
                return;
            }

            toast({ title: "Payment Initiated", description: "Please approve the transaction on your phone." });

            // 2. Proceed to Import (Optimistic for now, or we should wait for webhook?)
            // User requested "checkout to product imports", implying successful payment = import.
            // For better UX we proceed after initiation success, but normally we'd wait for 'success' status.

            // Simulating a short delay to allow user to check phone
            await new Promise(resolve => setTimeout(resolve, 2000));

            setImportingId(productToPay.id);
            setIsPaymentModalOpen(false); // Close modal

            // Call service to import
            const { data: { user } } = await supabase.auth.getUser();
            if (user) {
                const { error } = await importDropshipProduct(user.id, selectedStoreId, productToPay.id);
                if (error) {
                    toast({ title: "Import Failed", description: error.message, variant: "destructive" });
                } else {
                    toast({ title: "Product Imported", description: `${productToPay.name} has been added to your draft products.` });
                    router.push("/products?tab=draft");
                }
            }

        } catch (error: any) {
            toast({ title: "Error", description: "Something went wrong during checkout.", variant: "destructive" });
        } finally {
            setIsProcessingPayment(false);
            setImportingId(null);
            setProductToPay(null);
        }
    };
    ...
    const handleUseCredit = async () => {
        ...
        try {
            // Initiate free credit transaction
            const payResult = await initiateMarketImportPayment(selectedStoreId, productToPay.id, 'free_credit', '0000000000', 0);

            if (!payResult.success) {
                toast({ title: "Failed", description: payResult.message, variant: "destructive" });
                setIsProcessingPayment(false);
                return;
            }

            toast({ title: "Success", description: "Product imported using free credit!" });
            ...
            if (user) {
                ...
                if (error) {
                    ...
                    toast({ title: "Product Imported", description: `${productToPay.name} has been added to your draft products.` });
                    router.push("/products?tab=draft");
                    setImportCredits(prev => Math.max(0, prev - 1)); // Optimistic update
                }
            }
        } catch (e: any) {
    ...
    const avgMargin = React.useMemo(() => {
        ...
        const totalMargin = products.reduce((sum, p) => {
            ...
            const margin = price > 0 ? ((price - supplierPrice) / price) * 100 : 0;
            return sum + margin;
        }, 0);
    ...
    const activeSuppliersCount = React.useMemo(() => {
        const suppliers = new Set(products.map(p => p.store?.name).filter(Boolean));
    ...
    const filteredProducts = products
        ...
        .sort((a: any, b: any) => {
            ...
            if (sortOrder === "margin") return (b.price - b.supplier_price) - (a.price - a.supplier_price);
            return 0; // featured
        });
    ...
    return (
        <div className="container py-4 md:py-8 space-y-8 animate-in fade-in duration-500 min-h-screen">
            ...
            <PaymentModal
                ...
                amount={(productToPay?.supplier_price || productToPay?.price || 0) * 0.125}
                title={`Import ${productToPay?.name}`}
                description={`Pay import fee (12.5% of wholesale price: ZMW ${((productToPay?.supplier_price || productToPay?.price || 0) * 0.125).toFixed(2)}) to add this product to your store.`}
                description={`Pay import fee (12.5% of wholesale price: ZMW ${((productToPay?.supplier_price || productToPay?.price || 0) * 0.125).toFixed(2)}) to add this product to your store.`}
