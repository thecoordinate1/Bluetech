...
import { checkAccess } from './subscriptionService';
...
export async function getProfitSummaryStats(
  storeId: string
): Promise<{ data: ProfitSummaryStats | null; error: Error | null; accessDenied?: boolean }> {
  ...
  // 1. Get Vendor ID from Store (assuming caller should have verified this, but we need vendor_id for sub check)
  ...
  if (storeError || !store) {
    return { data: null, error: new Error("Could not verify store ownership.") };
  }

  // 2. Check Access
  const hasAccess = await checkAccess(store.vendor_id, 'advanced_analytics');
  if (!hasAccess) {
    console.warn(`[reportService.getProfitSummaryStats] Access denied for store ${storeId} (vendor ${store.vendor_id})`);
    return { data: null, error: new Error("Upgrade required to access profit analytics."), accessDenied: true };
  }
